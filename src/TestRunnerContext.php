...
final class TestRunnerContext implements SnippetAcceptingContext
{
    ...
    /** @var bool */
    protected $isAutogeneratedWorkingDirectory = false;
    ...
    public function createWorkingDirectory()
    {
        ...
        if (!$this->filesystem->exists($this->getWorkingDirectory())) {
            $this->filesystem->mkdir($this->getWorkingDirectory(), 0770);
        }
    ...
    {
        $this->filesystem->dumpFile($this->workingDirectory . '/' . $fileName, $content);
        $this->files[] = $this->workingDirectory . '/' . $fileName;
    }
     ...
     */
    public function iHaveTheConfiguration(PyStringNode $config): void 
    {
        $file = $this->workingDirectory . '/behat.yml';
        $this->createFile($file, $config->getRaw());
    }

    /**
     * @Given I have the feature:
     */
    public function iHaveTheFeature(PyStringNode $content): void
    {
     ...
     */
    public function iHaveTheContext(PyStringNode $definition): void 
    {
        $file = $this->workingDirectory . '/features/bootstrap/FeatureContext.php';
        $this->createFile($file, $definition->getRaw());
    }
     ...
     */
    public function iRunBehat($parameters = '', $phpParameters = ''): void 
    {
     ...
     */
    public function iHaveTheFileInDocumentRoot($filename, PyStringNode $content): void 
    {
     ...
     */
    public function iHaveAWebServerRunningOnAddressAndPort($hostname, $port): void 
    {
    ...
    private function runBehat(string $parameters = '', string $phpParameters = '')
    {
    ...
    private function runWebServer(string $hostname, string $port)
    {
